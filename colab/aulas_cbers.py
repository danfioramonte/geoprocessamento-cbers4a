...
CONVENÇÃO DE PASTAS (igual no Colab e no pixi local)
    ...
    data/interim/    reflectância, fusão, máscaras, S2, corregistro  (Aulas 01-03)
...
mesma grade de 2 m. É isso que faz o corregistro do AROSICS (Aula 03) fechar.
"""
...
# ao mesmo tempo, senão o corregistro da Aula 03 não tem o que casar.
...
def configurar_proj() -> None:
    """Fixa o PROJ/GDAL nos dados do ambiente conda. Só age no Colab + condacolab.

    O condacolab NÃO troca o Python do Colab: ele continua sendo /usr/bin/python3
    (sys.prefix = /usr) e só põe o site-packages do conda (/usr/local) na frente
    do sys.path. Sem as variáveis de ativação do conda, o pyproj e o rasterio
    procuram os dados em sys.prefix/share/proj — o proj.db do Ubuntu, de outra
    versão do PROJ — e falham com "proj_create: no database context specified".
    Aqui apontamos tudo para o share/ do próprio conda, achado pela localização
    do pyproj instalado. Chame ANTES de importar o rasterio.
    """
    if not no_colab():
        return
    ...
    if spec is None or not spec.origin:
        return
    # .../<prefixo>/lib/pythonX.Y/site-packages/pyproj/__init__.py
    prefixo = Path(spec.origin).resolve().parents[4]
    proj = prefixo / "share" / "proj"
    if not (prefixo / "conda-meta").exists() or not (proj / "proj.db").exists():
        return                       # pyproj do pip (wheel): ele já traz o próprio proj.db
    os.environ["PROJ_DATA"] = str(proj)
    os.environ["PROJ_LIB"] = str(proj)
    gdal = prefixo / "share" / "gdal"
    if gdal.exists():
        os.environ["GDAL_DATA"] = str(gdal)
    import pyproj.datadir
    pyproj.datadir.set_data_dir(str(proj))


# Pasta do curso no Google Drive. A Aula 00 grava aqui e as Aulas 01-03 leem
# daqui, então nada precisa ser arrastado entre sessões.
PASTA_DRIVE = "/content/drive/MyDrive/geoprocessamento-cbers4a/data"


def usar_drive(caminho=PASTA_DRIVE) -> Path:
    """No Colab, monta o Google Drive e passa a gravar/ler os dados nele.

    Fora do Colab não faz nada (continua em data/). O Drive montado sobrevive
    ao reinício do condacolab, então montar de novo é instantâneo.
    """
    if not no_colab():
        return _BASE
...
def alinhar_ms_pan(ms_path, pan_path):
    ...
    vem depois, inclusive o corregistro da Aula 03.
    """
    import rasterio
